document.addEventListener('DOMContentLoaded', () => {
    ...
    // BOTONNNNNNNN----------ANTERIOR
        ...
        // FUNCION PARA VANZAR SEMANAS CON BOTON----------
// function avanzarSemana() {
//         // Obtén todas las filas del calendario
//     const filas = document.querySelectorAll('#calendario-full tr');

//     // Encuentra la fila actual (la que contiene el día marcado)
//     let filaActual = null;
//     for (let fila of filas) {
//         const primerTd = fila.querySelector('td[data-fecha]');
//         if (primerTd && primerTd.classList.contains('current-day')) {
//             filaActual = fila;
//             break;
//         }
//     }

//     // Si encontramos la fila actual, movernos a la siguiente fila
//     if (filaActual) {
//         // Encuentra la siguiente fila (si no es la última)
//         const siguienteFila = filaActual.nextElementSibling;

//         if (siguienteFila) {
//             // Obtén el primer <td> con el atributo data-fecha en la siguiente fila
//             const siguienteTd = siguienteFila.querySelector('td[data-fecha]');

//             if (siguienteTd) {
//                // Establecer la fecha a la del primer td de la siguiente semana
//                 const nuevaFecha = new Date(siguienteTd.getAttribute('data-fecha'));
//                 fechaActual = nuevaFecha; // Actualiza la fecha actual

//                 // Marca este <td> como el día actual y actualiza el calendario
//                 actualizarCalendario(fechaActual);
//             }
//         } else {
...
// ME DEJA AVANZAR POR LAS SEMANA FALTA AJUSTARLO
// QUE NO SE SALGA DEL MES, SI PULSO AVAZAR DIA NO ENCUENTRA LA SEMANA TR Y NO ENSEÑA NADA
// NO ME ENCUTRA LA ULTIMA SEMANA DEL MES DE DEDICEIMBRE.
// ME LLEVA AL AL DIA SIGUIENTE QUE SE ENCUTRNA EN LA MISMA POSICON QUE EL DIA ACTUAL EJ, SI EST DOMINGO ME LLEVA AL DOMINGO DE LA SEMAN QUEVIENE
// function avanzarSemana() {
...
//     // Determinar el índice del `td` dentro de su fila
//     const celdas = Array.from(filaActual.children); // Obtener todos los `td` en el `tr` actual
//     const indiceTdActual = celdas.indexOf(tdActual);
...
//     // Obtener el `td` en la misma posición (índice) en la siguiente fila
//     const siguienteTd = siguienteFila.children[indiceTdActual];
//     if (!siguienteTd) {
//         alert("No hay un día correspondiente en la siguiente semana.");
//         return;
//     }

//     // Mover la clase `current-day` al nuevo `td`
//     tdActual.classList.remove('current-day');
//     siguienteTd.classList.add('current-day');
...
// ESTA FUNCION ME LLEVA A LA MISMA POSICON PERO DE LA SEMANA QUE VIENE, Y SI NO HAY UN DIA EN LA MISMA POSICON, BUSCA EL PRIMER TD QUE ENCUENTRE EN UN TR:
// function avanzarSemana() {
//     // Obtener todos los `tr` del calendario
//     const filas = document.querySelectorAll('#calendario-full tr');
...
//     // Buscar un día válido en la siguiente fila
//     let siguienteTd = siguienteFila.children[indiceTdActual];
//     if (!siguienteTd || siguienteTd.textContent.trim() === '') {
//         // Si no hay celda válida, buscar la primera celda válida en la fila siguiente
//         siguienteTd = Array.from(siguienteFila.children).find(celda => celda.textContent.trim() !== '');
//         if (!siguienteTd) {
